  ...
  if (selected.costing.missingItems.length === 0) {
    return (
      <section className="card" style={{ marginBottom: 0 }}>
        <h3 style={{ marginTop: 0 }}>Items sin costo</h3>
        <p className="muted">Sin faltantes. Todos los items del desglose tienen costo vigente.</p>
      </section>
    );
  }
  ...
  return (
    <section className="card" style={{ marginBottom: 0 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8 }}>
        <h3 style={{ marginTop: 0, marginBottom: 8 }}>Items sin costo</h3>
      ...
      <ul className="costingDrawerMissingList">
        {selected.costing.missingItems.map((item) => (
          <li key={item.id} className="costingDrawerMissingList__item">
            ...
            <Link className="btnSecondary" href={`/items/${item.id}`}>Abrir item</Link>
          </li>
        ))}
      </ul>
    </section>
