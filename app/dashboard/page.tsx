...
export default function DashboardPage() {
  ...
  const [manualCostForms, setManualCostForms] = useState<
    Record<string, { open: boolean; costGrossClp: string; validFrom: string; error: string | null }>
  >({});
  ...
  function openSetCostoManual(productId: string): void {
    setManualCostForms((prev) => ({
      ...prev,
      [productId]: {
        open: true,
        costGrossClp: prev[productId]?.costGrossClp ?? '',
        validFrom: prev[productId]?.validFrom ?? todayIsoDate(),
        error: null,
      },
    }));
  }

  function updateSetCostoManual(
    productId: string,
    patch: Partial<{ open: boolean; costGrossClp: string; validFrom: string; error: string | null }>,
  ): void {
    setManualCostForms((prev) => ({
      ...prev,
      [productId]: {
        open: prev[productId]?.open ?? false,
        costGrossClp: prev[productId]?.costGrossClp ?? '',
        validFrom: prev[productId]?.validFrom ?? todayIsoDate(),
        error: prev[productId]?.error ?? null,
        ...patch,
      },
    }));
  }

  function onSaveManualCostSantiago(productId: string): void {
    const form = manualCostForms[productId];
    if (!form) {
      return;
    }

    const amount = Number(form.costGrossClp);
    const validFrom = new Date(`${form.validFrom}T00:00:00.000Z`);

    if (!Number.isFinite(amount) || amount < 0) {
      updateSetCostoManual(productId, { error: 'Costo manual inválido (>= 0).' });
      return;
    }

    if (!form.validFrom || Number.isNaN(validFrom.getTime())) {
      updateSetCostoManual(productId, { error: 'Fecha válida requerida.' });
      return;
    }

    try {
      addProductCostVersion(productId, 'Santiago', {
        costGrossClp: Math.round(amount),
        validFrom,
      });
      updateSetCostoManual(productId, {
        open: false,
        error: null,
        costGrossClp: '',
      ...
      updateSetCostoManual(productId, {
        error: error instanceof Error ? error.message : 'No se pudo guardar costo manual.',
      });
    }
  }
              ...
              <tr key={`alerta-${row.productId}`}>
                ...
                <td style={{ padding: 8, borderBottom: '1px solid #eee' }}>
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    <button type="button" onClick={() => openSetCostoManual(row.productId)}>
                      Set costo manual
                    </button>
                  </div>
                  {manualCostForms[row.productId]?.open ? (
                    <div style={{ marginTop: 8, display: 'grid', gap: 6, maxWidth: 320 }}>
                      <label>
                        ...
                        <input
                          ...
                          min="0"
                          step="1"
                          value={manualCostForms[row.productId]?.costGrossClp ?? ''}
                          onChange={(event) =>
                            updateSetCostoManual(row.productId, {
                              costGrossClp: event.target.value,
                              error: null,
                            })
                          }
                        />
                      ...
                      <label>
                        ...
                        <input
                          type="date"
                          value={manualCostForms[row.productId]?.validFrom ?? todayIsoDate()}
                          onChange={(event) =>
                            updateSetCostoManual(row.productId, {
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button type="button" onClick={() => onSaveManualCostSantiago(row.productId)}>
                          Guardar
                        </button>
                        <button
                          type="button"
                          onClick={() => updateSetCostoManual(row.productId, { open: false, error: null })}
                        >
                          Cancelar
                        </button>
                      </div>
                      {manualCostForms[row.productId]?.error ? (
                        <p style={{ color: 'crimson', margin: 0 }}>{manualCostForms[row.productId]?.error}</p>
                      ) : null}
                    </div>
                  ) : null}
                </td>
